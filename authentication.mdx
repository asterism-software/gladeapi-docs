---
title: "Glade API Authentication: API Keys and Request Headers"
sidebarTitle: "Authentication"
description: "Glade API uses API keys for authentication. Learn how to generate a key, pass it in requests using supported headers, and handle authentication errors."
---
...
REST and GraphQL accept either `API-KEY` or a Bearer token. MCP accepts those two formats plus `GLADE-API-KEY` and `X-API-KEY`.
...
```bash
curl "https://gladeapi.com/api/amazon/product?asin=B0D1XD1ZV3&domain=US" \
  -H "Authorization: Bearer YOUR_GLADE_API_KEY"
```
...
curl -X POST "https://gladeapi.com/api/mcp" \
  -H "GLADE-API-KEY: YOUR_GLADE_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{ ... }'

# Using X-API-KEY
curl -X POST "https://gladeapi.com/api/mcp" \
  -H "X-API-KEY: YOUR_GLADE_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{ ... }'
```
...
```json
{
  "success": false,
  "errors": [
    { "code": 7003, "message": "Unauthorized" }
  ]
}
```
...
| `403 Forbidden` | The key lacks the `amazon:read` scope, or your plan does not include GraphQL or MCP. | Use a key with the required scope or choose a plan that includes the interface. |
...
```bash
export GLADE_API_KEY=YOUR_GLADE_API_KEY
```
